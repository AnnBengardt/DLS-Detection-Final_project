...
def custom_yolov5s(device="CPU"):
    ...
    st.text("В перспективе такую модель можно использовать на мусороперерабатывающем заводе для\n быстрой сортировки или для дронов, собирающих мусор в природных зонах вроде лесов\n и океанов.")
...
def video_custom_yolov5s():
    ...
    uploaded_video = None     #st.file_uploader("Загрузить видео", type=['mp4', 'mpeg', 'mov'])
    if uploaded_video != None:
        ...
        subprocess.run( ["ffmpeg", "-i", outputpath, "-vcodec", "libx264", "-f", "mp4", outputpath] )
